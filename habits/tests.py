...
class HabitTestCase(APITestCase):
    ...
    def test_habit_update_admin_access(self):
        ...
        self.client.force_authenticate(user=self.admin_user)
        url = reverse("habits:habit-detail", args=(self.useful_habit_with_related_habit.pk,))
        data = {"place": "на улице"}
    ...
    def test_habit_create_PeriodicityValidator_error(self):
        ...
        self.assertRaises(ValidationError)
        self.assertRaisesMessage(ValidationError, "Нельзя выполнять привычку реже 1 раза в 7 дней.")
