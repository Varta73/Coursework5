...
def send_habit_reminder():
    ...
    if habits:
        for habit in habits:
            message_1 = (f"У вас запланирована привычка: "
                         f"{habit.user.first_name if habit.user.first_name else habit.user.email}, "
                         f"через 5 минут пора {habit.action} {habit.place}!"
                         f" ({timezone.localtime(habit.do_at).strftime("%d.%m.%Y %H:%M")})")
            ...
            if habit.user.tg_chat_id:
                ...
                send_telegram_message.apply_async(args=[habit.user.tg_chat_id, message_2], countdown=60)

                # Отправка напоминания о выполнении приятной привычки при ее наличии
                if habit.related_habit:
                    enjoyable_habit = habit.related_habit
                    message = f"Вы молодец! Настало время сделать что-то приятное. Пора {enjoyable_habit.action}!"
                ...
                # Отправка напоминания о вознаграждении при его наличии
                    ...
                    message = f"Вы молодец! Настало время получить вознаграждение. Пора {habit.reward}!"
                    send_telegram_message.apply_async(
                        args=[habit.user.tg_chat_id, message],
                        countdown=(habit.duration + 5 * 60),
                    )
